...
записан не в ту кассу (например, все штамповались через "O", хотя
часть реально сдавалась консьержам K1-K6 или инкассаторам).

Синхронно правит payments и связанную cashbox_operations, и
пересчитывает баланс ОБЕИХ касс — старой (откуда убрали сумму) и
новой (куда она реально попадает) — тем же проверенным способом,
что и остальные наши инструменты сегодня.

Использование:
    python fix_payment_cashbox.py PAYMENT_ID НОВЫЙ_КОД

Пример:
    python fix_payment_cashbox.py 100 Central
...
def main():
    if len(sys.argv) < 3:
        print("Использование: python fix_payment_cashbox.py PAYMENT_ID НОВЫЙ_КОД")
        print("Пример: python fix_payment_cashbox.py 100 Central")
        return
